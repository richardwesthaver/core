...
(deftest sanity ()
  (mapc (lambda (x) (is (= (car x) (cdr x))))
        `((,zstd::+zstd-clevel-default+ . ,(zstd::zstd-defaultclevel))
  ...
  (let ((test "test 1 2 3"))
    (with-zstd-buffers (in out :src (make-alien-string test))
      (with-zstd-streams (cs ds)
        (zstd-compressstream2 cs out in :continue)
        (zstd-compressstream2 cs out in :flush)
        (is (zerop (zstd-iserror (zstd-compressstream2 cs out in :end))))
        (zstd::zstd-flushstream cs out)
