...
(defun test-single-roundtrip (db &optional (size 32))
  (let* ((k (random-bytes size))
         (v (random-bytes size)))
    (with-alien ((.k (* unsigned-char) (make-alien unsigned-char size))
                 (.v (* unsigned-char) (make-alien unsigned-char size)))
      (setfa .k k)
      (setfa .v v)
      (with-errptr e
    ...
    (with-alien ((k (* unsigned-char) (make-alien unsigned-char klen))
      ...
      (with-errptr errptr
        (let ((db (rocksdb-open opts path errptr)))
          ...
          (rocksdb-close db)
          (with-errptr e
            (rocksdb-destroy-db opts path e))
          (rocksdb-options-destroy opts))))))
...
(deftest blob ()
  ...
  (let* ((opts (test-opts))
    ...
    (setf db (with-errptr e (rocksdb-open opts path e)))
    (with-alien ((k (* (unsigned 8)) (make-alien (unsigned 8) klen))
                 (v (* (unsigned 8)) (make-alien (unsigned 8) vlen)))
...
(deftest iterators ()
  ...
  (with-opt (o (test-opts) (rocksdb-options-destroy o))
    (with-temp-db db (o)
      (let ((k (octets-to-alien (string-to-octets "foo")))
            (v (octets-to-alien (string-to-octets "bar"))))
        (with-errptr e (rocksdb-put db (rocksdb-writeoptions-create) k 3 v 3 e))
        (with-errptr e (rocksdb-flush db (rocksdb-flushoptions-create) e))
        (with-alien ((iter (* rocksdb-iterator) (rocksdb-create-iterator db (rocksdb-readoptions-create))))
