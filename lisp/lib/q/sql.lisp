...
(defclass sql-select (sql-relation)
  ((projection :type sql-expression-vector)
   (selection :type (or sql-expression null))
   (group-by :type sql-expression-vector)
   (order-by :type sql-expression-vector)
   (having :type (or null sql-expression))
...
(defun num-start-p (c) (or (digit-char-p c) (char= #\. c)))
...
(defun next-sql-token (stream)
  ...
  (block :next
    (let ((tok)
      ...
      (cond
        ((num-start-p next) nil)
        ((ident-start-p next) nil)
        ((str-start-p next) nil)
    ...
    (loop while sort
          do (progn
               (etypecase (sql-token-type sort)
                 (sql-identifier (setf sort (make-instance 'sql-sort :exprt sort :asc t)))
                 (sql-sort nil))
               (push sort sort-list)
