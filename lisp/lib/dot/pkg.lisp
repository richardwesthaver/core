...
;; (defclass id-mixin ()
;;   ((id :initform (incf *id*) :initarg :id :accessor id)))
    ...
    (let ((*standard-output* (or stream *standard-output*))
      ...
      (maphash
       (lambda (cluster nodes)
         (if (null cluster)
             (dolist (node nodes)
               (format stream "  ~a " (textify (id node)))
               (print-attributes stream (attributes-of node) *node-attributes*)
               (format stream ";~%"))
             (progn
               (format stream "  subgraph cluster_~d {~%" (id cluster))
      ...
      (dolist (edge edges)
        (format stream "  ~a~@[:~a~] ~a ~a~@[:~a~]"
                (textify (id (source-of edge))) (source-port-of edge)
                edge-op
                (textify (id (target-of edge))) (target-port-of edge))
