...
(defun get-auth-request-url (auth-server
  ...
  (make-instance 'uri
   :path
   (slot-value auth-server 'auth-url)
   :query
   (delete nil
           (apply #'list
...
(defun %acquire-token-auth-code
  ...
  (req:post (slot-value (slot-value oauth 'auth-server) 'token-url)
...
(defun refresh-token-json (oauth); &key (who #'refresh-token-json))
  ...
  (req:post
   (slot-value (slot-value oauth 'auth-server) 'token-url)
   :headers (cons "Content-Type" "application/x-www-form-urlencoded")
...
(defun oauth2-auth-code (auth-server client auth-code &key redirect-uri)
...
(defun request-auth-code-with-browser (auth-server client
                                       &key
                                       (scopes nil)
                                       (port 5000))
  "Given an auth-server-config an auth-client object, and a list of strings
