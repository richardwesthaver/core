...
(defclass toml-table () ((table :initform (make-hash-table :test 'equal)
                                :initarg :table)))
  
(defclass toml-document (ast) ())
...
(defun toml-read-datetime (stream)
  (parse-timestring (ppcre:regex-replace " " "T" (read stream))))

(defmethod toml-read-datetime-local (stream)
  (let* ((val (read stream))
         (delimeter (sequence:elt val 10))
         (splits (split-sequence delimeter val)))
