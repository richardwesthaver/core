...
(defun init-skel-db-logger (&optional (cfg *skel-logger-config*))
  (unless (and (boundp '*skel-logger*) (null *skel-logger*))
    (let ((lgr (build cfg)))
      (with-db (db :db (sink lgr) :close :auto :open nil)
...
;; (sk-log nil "hello world")


;; (with-db (db :db (sink *skel-logger*))
