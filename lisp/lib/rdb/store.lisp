        ...
        (multiple-value-bind (key pkey val)
          ...
          (when (and key (= (buffer-read-oid key) (cursor-oid cursor))
                     (= (buffer-read-oid pkey) (oid (primary
                                                     (cursor-btree cursor)))))
            (delete-key (deserialize pkey (get-store (cursor-btree cursor)))
  ...
  (when (cursor-initialized-p cursor)
    (with-static-streams (key-buf value-buf)
      (multiple-value-bind (key val)
        ...
        (if (and key (= (buffer-read-oid key) (cursor-oid cursor)))
            (values t (deserialize key (get-store (cursor-btree cursor))) 
                    (deserialize val (get-store (cursor-btree cursor))))
            (setf (cursor-initialized-p cursor) nil))))))
      ...
      (multiple-value-bind (key pkey val)
        ...
        (if (and key (= (buffer-read-oid key) (cursor-oid cursor)))
            (the (values t t t t)
              (values t (deserialize key (get-store (cursor-btree cursor)))
                      (deserialize val (get-store (cursor-btree cursor)))
                      (progn (buffer-read-oid pkey) (deserialize pkey (get-store (cursor-btree cursor))))))
            (the null (setf (cursor-initialized-p cursor) nil)))))))
        ...
        (multiple-value-bind (key pkey val)
          ...
          (if (and key (= (buffer-read-oid key) (cursor-oid cursor)))
              (values t (deserialize key (get-store (cursor-btree cursor)))
                      (deserialize val (get-store (cursor-btree cursor)))
                      (progn (buffer-read-oid pkey)
                             (deserialize pkey (get-store (cursor-btree cursor)))))
              (setf (cursor-initialized-p cursor) nil))))
      (cursor-plast cursor)))
