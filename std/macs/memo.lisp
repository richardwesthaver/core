  ...
  (with-gensyms (table value exists-p args)
    (labels ((transformer (x)
               (cond
                 ...
                 ((and (listp x) (eql (car x) 'memoizing))
                  (let ((body (cdr x)))
                    (cond
                      ((and (listp body) (eql (car body) 'cl:let))
                      ...
                      ((and (listp body) (or (eql (car body) 'cl:defun) (eql (car body) 'cl:defmethod)))
                       (let ((def (car body))
                         ...
                         (multiple-value-bind (body declares) (parse-body (cdddr body))
                           (setf need-hashtablep t)
                           (assert (not (intersection '(&rest &allow-other-keys) args)) nil "can't memoize functions with &rest, &allow-other-keys in their defining lambda-lists")
                           `(,def ,name (,@args)
                                ,@declares
                              (letv* ((,args (list ',id ,@(mapcar #'(lambda (x) (first (std/list:ensure-list x))) (set-difference args cl:lambda-list-keywords))))
                                      (,value ,exists-p (gethash ,args ,table)))
                                (values-list
                                 (if ,exists-p ,value
                                     (setf (gethash ,args ,table) (multiple-value-list (progn ,@body))))))))))
                      ((and (listp body) (or (eql (car body) 'cl:labels) (eql (car body) 'cl:flet)))
                       (let ((def (car body))
