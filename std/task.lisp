...
;; taskstamp?
(deftype tstamp () '(or real boolean))
(defun tstamp< (x y)
  (etypecase x
    ((eql t) (not (eql y t)))
    (real (etypecase y
            ...
            (null t)))
    (null nil)))
(defun tstamps< (list) (loop :for y :in list :for x = nil :then y :always (tstamp< x y)))
(defun tstamp*< (&rest list) (tstamps< list))
(defun tstamp<= (x y) (not (tstamp< y x)))
(defun earlier-tstamp (x y) (if (tstamp< x y) x y))
(defun tstamps-earliest (list) (reduce 'earlier-tstamp list :initial-value nil))
(defun earliest-tstamp (&rest list) (tstamps-earliest list))
(defun later-tstamp (x y) (if (tstamp< x y) y x))
(defun tstamps-latest (list) (reduce 'later-tstamp list :initial-value t))
(defun latest-tstamp (&rest list) (tstamps-latest list))
(define-modify-macro latest-tstamp-f (&rest tstamps) latest-tstamp)
...
(defun merge-status (status1 status2) ;; status-and
  ...
  (make-status
   ...
   :stamp (latest-tstamp (status-stamp status1) (status-stamp status2))
