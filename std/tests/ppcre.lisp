...
(deftest ppcre ()
  (isequalp (multiple-value-list (scan "(a)*b" "xaaabd"))
            (list 1 5 #(3) #(4)))

  (isequalp (multiple-value-list (scan "(a)*b" "xaaabd" :start 1))
            (list 1 5 #(3) #(4)))

  (isequalp (multiple-value-list (scan "(a)*b" "xaaabd" :start 2))
            (list 2 5 #(3) #(4)))

  (null (scan "(a)*b" "xaaabd" :end 4))

  (isequalp (multiple-value-list (scan '(:greedy-repetition 0 nil #\b) "bbbc"))
            (list 0 3 #() #()))

  (null (scan '(:greedy-repetition 4 6 #\b) "bbbc"))

  (let ((s (create-scanner "(([a-c])+)x")))
    (isequalp (multiple-value-list (scan s "abcxy"))
              (list 0 4 #(0 2) #(3 3))))

  (isequalp (multiple-value-list (scan-to-strings "[^b]*b" "aaabd"))
            (list "aaab" #()))
  ...
  (isequalp (register-groups-bind (first second third fourth)
                ("((a)|(b)|(c))+" "abababc" :sharedp t)
              (list first second third fourth))
            (list "c" "a" "b" "c"))

  (isequalp (register-groups-bind (nil second third fourth)
                ("((a)|(b)|(c))()+" "abababc" :start 6)
              (list second third fourth))
            (list nil nil "c"))

  (isnt (register-groups-bind (first)
            ("(a|b)+" "accc" :start 1)
          first))

  (isequalp (register-groups-bind (fname lname (#'parse-integer date month year))
  ...
  (isequal (count-matches "a" "foo bar baz")
           2)

  (isequal (count-matches "\\w*" "foo bar baz")
           6)

  (isequal (all-matches "a" "foo bar baz")
           (list 5 6 9 10))

  (isequal (all-matches "\\w*" "foo bar baz")
           (list 0 3 3 3 4 7 7 7 8 11 11 11))

  (isequal (all-matches-as-strings "a" "foo bar baz")
           (list "a" "a"))

  (isequal (all-matches-as-strings "\\w*" "foo bar baz")
           (list "foo" "" "bar" "" "baz" ""))

  (isequal (split "\\s+" "foo   bar baz
frob")
           '("foo" "bar" "baz" "frob"))
  ...
  (isequal (split "(\\s+)" "foo bar   baz" :with-registers-p t)
           '("foo" " " "bar" "   " "baz"))

  (isequal (split "(\\s)(\\s*)" "foo bar   baz" :with-registers-p t)
           '("foo" " " "" "bar" " " "  " "baz"))

  (isequal (split "(,)|(;)" "foo,bar;baz" :with-registers-p t)
           '("foo" "," nil "bar" nil ";" "baz"))

  (isequal (split "(,)|(;)" "foo,bar;baz" :with-registers-p t :omit-unmatched-p t)
           '("foo" "," "bar" ";" "baz"))

  (isequal (split ":" "a:b:c:d:e:f:g::")
           '("a" "b" "c" "d" "e" "f" "g"))

  (isequal (split ":" "a:b:c:d:e:f:g::" :limit 1)
           '("a:b:c:d:e:f:g::"))

  (isequal (split ":" "a:b:c:d:e:f:g::" :limit 2)
           '("a" "b:c:d:e:f:g::"))

  (isequal (split ":" "a:b:c:d:e:f:g::" :limit 3)
           '("a" "b" "c:d:e:f:g::"))

  (isequal (split ":" "a:b:c:d:e:f:g::" :limit 1000)
           '("a" "b" "c" "d" "e" "f" "g" "" ""))

  (isequal (split ":" "a:b:c:d:e:f:g::" :limit -1)
           '("a" "b" "c" "d" "e" "f" "g" "" ""))

  (isequal (multiple-value-list (regex-replace "fo+" "foo bar" "frob"))
           (list "frob bar" t))

  (isequal (multiple-value-list (regex-replace "fo+" "FOO bar" "frob"))
           (list "FOO bar" nil))

  (isequal (multiple-value-list (regex-replace "(?i)fo+" "FOO bar" "frob"))
           (list "frob bar" t))

  (isequal (multiple-value-list (regex-replace "(?i)fo+" "FOO bar" "frob" :preserve-case t))
           (list "FROB bar" t))

  (isequal (multiple-value-list (regex-replace "(?i)fo+" "Foo bar" "frob" :preserve-case t))
           (list "Frob bar" t))

  (isequal (multiple-value-list (regex-replace "bar" "foo bar baz" "[frob (was '\\&' between '\\`' and '\\'')]"))
           (list "foo [frob (was 'bar' between 'foo ' and ' baz')] baz" t))

  (isequal (multiple-value-list
            (regex-replace "bar" "foo bar baz"
                           '("[frob (was '" :match "' between '" :before-match "' and '" :after-match "')]")))
           (list "foo [frob (was 'bar' between 'foo ' and ' baz')] baz" t))

  (isequal (multiple-value-list (regex-replace "(be)(nev)(o)(lent)"
  ...
  (isequal (multiple-value-list (regex-replace-all "(?i)fo+" "foo Fooo FOOOO bar" "frob" :preserve-case t))
           (list "frob Frob FROB bar" t))

  (is (string= (regex-replace-all "(?i)f(o+)" "foo Fooo FOOOO bar" "fr\\1b" :preserve-case t)
  ...
  (let ((qp-regex (create-scanner "[\\x80-\\xff]")))
    (flet ((encode-quoted-printable (string)
      ...
      (is (string= (encode-quoted-printable "Fête Sørensen naïve Hühner Straße")
  ...
  (let ((url-regex (create-scanner "[^a-zA-Z0-9_\\-.]")))
    (flet ((url-encode (string)
      ...
      (is (string= (url-encode "Fête Sørensen naïve Hühner Straße")
...
Version using SIMPLE-CALLS keyword argument."
  ...
  (let ((*allow-quoting* t))
    ...
    (isequalp (multiple-value-list (scan (create-scanner "^\\Qa+\\E$") "a+"))
              (list 0 2 #() #())))

  (is (string= (parse-string "\\k<reg>") "k<reg>"))

  (let ((*allow-named-registers* t))
    (isequal (nth-value 1 (create-scanner "((?<small>[a-z]*)(?<big>[A-Z]*))"))
             (list nil "small" "big")))

  (let ((*allow-named-registers* t))
    (isequal (nth-value 1 (create-scanner '(:register
                                            (:sequence
  ...
  (let ((*allow-named-registers* t))
    (isequalp (multiple-value-list (scan (create-scanner "((?<small>[a-z]*)(?<big>[A-Z]*))") "aaaBBB"))
              (list 0 6 #(0 0 3) #(6 3 6))))

  (let ((*allow-named-registers* t))
    ;; multiple-choice back-reference
    (isequalp (multiple-value-list (scan (create-scanner "^(?<reg>[ab])(?<reg>[12])\\k<reg>\\k<reg>$") "a1aa"))
              (list 0 4 #(0 1) #(1 2))))

  (let ((*allow-named-registers* t))
    (isequalp (multiple-value-list (scan (create-scanner "^(?<reg>[ab])(?<reg>[12])\\k<reg>\\k<reg>$") "a22a"))
              (list 0 4 #(0 1) #(1 2))))

  (let ((*allow-named-registers* t))
    ;; demonstrating most-recently-seen-register-first property of back-reference;
    ;; "greedy" regex (analogous to "aa?")
    (isequalp (multiple-value-list (scan (create-scanner "^(?<reg>)(?<reg>a)(\\k<reg>)") "a"))
              (list 0 1 #(0 0 1) #(0 1 1))))

  (let ((*allow-named-registers* t))
    (isequalp (multiple-value-list (scan (create-scanner "^(?<reg>)(?<reg>a)(\\k<reg>)") "aa"))
              (list 0 2 #(0 0 1) #(0 1 2))))

  (let ((*allow-named-registers* t))
    ;; switched groups
    ;; "lazy" regex (analogous to "aa??")
    (isequalp (multiple-value-list (scan (create-scanner "^(?<reg>a)(?<reg>)(\\k<reg>)") "a"))
              (list 0 1 #(0 1 1) #(1 1 1))))

  (let ((*allow-named-registers* t))
    ;; scanner ignores the second "a"
    (isequalp (multiple-value-list (scan (create-scanner "^(?<reg>a)(?<reg>)(\\k<reg>)") "aa"))
              (list 0 1 #(0 1 1) #(1 1 1))))
  ...
  (is (string= (quote-meta-chars "[a-z]*") "\\[a\\-z\\]\\*"))

  (is (string= (handler-case
...
characters if there's a match."
           (cond ((and (oddp pos)
                       (< pos ppcre::*end-pos*)
                       (lower-case-p (char ppcre::*string* pos)))
                  (1+ pos))
                 ((and (evenp pos)
                       (< (1+ pos) ppcre::*end-pos*)
                       (upper-case-p (char ppcre::*string* pos))
                       (upper-case-p (char ppcre::*string* (1+ pos))))
                  (+ pos 2))
                 (t nil))))
    (let ((weird-regex `(:sequence "+" (:filter ,#'my-weird-filter) "+")))
      (isequalp (multiple-value-list (scan weird-regex "+A++a+AA+"))
                (list 5 9 #() #()))))

  (let ((a "\\E*"))
    (isequalp (multiple-value-list (scan (concatenate 'string "(?:" (quote-meta-chars a) "){2}") "\\E*\\E*"))
              (list 0 6 #() #())))

  (let ((a "\\E*"))
    (isequalp (multiple-value-list (scan `(:greedy-repetition 2 2 ,a) "\\E*\\E*"))
              (list 0 6 #() #())))

  (loop for *optimize-char-classes* in '(:hash-table :hash-table* :charset :charset* :charmap)
        for s = (create-scanner "(([a-c])+)x")
        always (isequalp (multiple-value-list (scan s "abcxy"))
                         (list 0 4 #(0 2) #(3 3))))
  ...
  (macrolet ((z () "(a)*b"))
    (isequalp (multiple-value-list (scan (z) "xaaabd"))
              (list 1 5 #(3) #(4))))

  (macrolet ((z () "[^b]*b"))
    (isequalp (multiple-value-list (scan-to-strings (z)  "aaabd"))
              (list "aaab" #())))

  (let ((s (create-scanner '(:sequence ""))))
    (isequalp (multiple-value-list (scan s "abcfooxy"))
              (list 0 0 #() #())))
