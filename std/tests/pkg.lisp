...
(eval-always
  (deferror testing-error (simple-error) nil (:reporter t) (:documentation "testing"))
...
(deftest fv ()
  (iseql 'unsigned-char (element-type-to-alien 'octet))
  (let ((fv (make-instance (foreign-vector 'octet) :length 4096)))
    (iseql 'octet (foreign-vector-element-type fv))
    (is= 4096 (foreign-vector-length fv))
    (alloc fv)
    (is= 1(setf (fvref fv 0) 1))
    (is= 1 (fvref fv 0))
    (is= 255 (setf (fvref fv 0) 255))
    (is= 0 (fvref fv 4095))
