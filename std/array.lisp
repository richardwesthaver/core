...
(eval-every
  (let ((code `(lambda (val lb ub vec &key (order #'<) (test #'=))
    ...
    (define-compiler-macro binary-search (&whole form val lb ub vec &key (order '(function <)) (test '(function =)))
      (declare (ignorable form val lb ub vec))
      (let ((t-code (maptree-eki #'(lambda (x)
                                     ...
                                     (cond
                                       ...
                                       ((and (listp x) (eql (car x) 'funcall))
                                        (destructuring-bind (fname &rest argvs) (cdr x)
  ...
  sequence @arg{seq} when applied.")
    (define-compiler-macro sort-index (&whole form seq predicate &key key)
      ...
      (let ((t-code (maptree-eki #'(lambda (x)
                                     (cond
                                       ((and (listp x) (= (length x) 3))
                                        (destructuring-bind (type vector seq) x
                                          (declare (ignore vector))
                                          (if (and (listp seq) (eql (car seq) 'the))
                                              `(type ,type seq))
                                          x))
                                       ((and (listp x) (eql (car x) 'funcall) (>= (length x) 2)
                                             (destructuring-bind (predicate &rest argvs) (cdr x)
                                               (cond
                                                 ((and (listp predicate)
                                                       (or (eql (car predicate) 'function)
                                                           (eql (car predicate) 'quote))
                                                       (typep (second predicate) 'symbol))
                                                  `(,(second predicate) ,@argvs))
                                                 ((listp predicate)
                                                  (cond
                                                    ((and (eql (car predicate) 'function)
                                                          (listp (second predicate))
                                                          (eql (cadr predicate) 'lambda)
                                                          (listp (cdadr predicate)))
                                                     (let ((args (cadadr predicate))
                                                           (body (cdr (cdadr predicate))))
                                                       `(let (,@(zip args argvs)) ,@body)))
                                                    ((and (eql (car predicate) 'lambda)
                                                          (listp (second predicate)))
                                                     (let ((args (second predicate))
                                                           (body (cddr predicate)))
                                                       `(let (,@(zip args argvs)) ,@body)))))
                                                 (t x)))))
                                       ((and (listp x) (eql (car x) 'funcall) (eql (second x) 'key))
                                        (let ((argvs (cddr x))
                                              (y (or key '(lambda (x) x))))
                                          (cond
                                            ((and (listp y) (or (eql (car y) 'function) (eql (car y) 'quote))
                                                  (typep (second y) 'symbol)) 
                                             `(,(second y) ,@argvs))
                                            ((listp y)
                                             (cond 
                                               ((and (eql (car y) 'function) (listp (second y))
