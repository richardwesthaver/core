...
represent the fractional parts of a second."
  (multiple-value-bind (secs usecs)
      (sb-ext:get-time-of-day)
    (the timetag (unix-timetag secs usecs))))
...
(defun unix-time-to-timetag (unix-time)
  (multiple-value-bind (secs subsecs)
      (floor unix-time)
    (the timetag
         (unix-timetag secs
                       (subsecs-to-microseconds subsecs)))))

(defun timetag-to-unix-time (timetag)
  (if (= timetag 1)
      1                                 ; immediate timetag
...
(defun microseconds-to-subsecs (usecs)
  (declare (type (integer 0 1000000) usecs))
  (coerce (/ usecs  +usecs+) 'double-float))

(defun subsecs-to-microseconds (subsecs)
