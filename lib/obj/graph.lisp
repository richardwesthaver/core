...
(defgeneric edge-in (self)
  (:method ((self cons)) (car self)))
(defgeneric (setf edge-in) (new self) 
  (:method (new (self cons)) (setf (car self) new)))
(defgeneric edge-out (self)
  (:method ((self cons)) (cdr self)))
(defgeneric (setf edge-out) (new self)
  (:method (new (self cons)) (setf (cdr self) new)))

(defclass edge (node)
  ((in :initarg :in :accessor edge-in) (out :initarg :out :accessor edge-out))
  (:documentation "generic edge mixin. Compatible with the NODE protocol."))
...
(defmethod add-edge ((graph simple-graph) (edge list) &optional value)
  (mapc (lambda (node)
          (add-node graph node)
          (pushnew 
           (typecase graph
