;;; bool.lisp --- Boolean Tensors

;; 

;;; Code:
(in-package :obj/tensor)

;;Define boolean tensor
(eval-every
  (tensor 'boolean)
  (define-template-method (t.store-allocator #'(lambda (x) (eql (field-type x) 'boolean))) (type simple-vector-store-mixin) (size &rest initargs)
    `(t.store-allocator ,(tensor 'bit) ,size ,@initargs))
  (define-template-method (t.store-type #'(lambda (x) (member (field-type x) '(bit boolean)))) (type simple-vector-store-mixin) (&optional (size '*))
    `(simple-bit-vector ,size))
  (define-template-method (t.store-ref #'(lambda (x) (eql (field-type x) 'boolean))) (type simple-vector-store-mixin) (store &rest idx)
    (with-gensyms (out)
      `(lety ((,out (t.store-ref ,(tensor 'bit) ,store ,@idx) :type bit))
         (the boolean (case ,out (1 t))))))
  (define-template-method (t.store-set #'(lambda (x) (eql (field-type x) 'boolean))) (type simple-vector-store-mixin) (value store &rest idx)
    `(setf (t.store-ref ,(tensor 'bit) ,store ,@idx) (the bit (if ,value 1 0)))))

(define-tensor-generic ge= (a b)
  (:method ((a tensor) (b tensor))
    (assert (vector-eq (dimensions a) (dimensions b)) nil 'tensor-dimension-mismatch)))
(define-tensor-generic ga= (a b)
  (:method ((a tensor) (b tensor))
    (assert (vector-eq (dimensions a) (dimensions b)) nil 'tensor-dimension-mismatch)))
(define-tensor-generic go= (a b)
  (:method ((a tensor) (b tensor))
    (assert (vector-eq (dimensions a) (dimensions b)) nil 'tensor-dimension-mismatch)))

(define-tensor-method ge= (a (b dense-tensor :x))
  `(let ((a (t.coerce ,(field-type (cl :x)) a))
         (ret (zeros (dimensions b) (tensor 'boolean))))
     (dorefs (idx (dimensions b))
             ((ref.b b :type ,(cl :x))
              (ref.r ret :type ,(tensor 'boolean)))
             (setf ref.r (t.f= ,(field-type (cl :x)) a ref.b)))
     ret))

(define-tensor-method ge= ((a dense-tensor :x) (b dense-tensor :x))
  `(let ((ret (zeros (dimensions a) (tensor 'boolean))))
     (dorefs (idx (dimensions a))
             ((ref.a a :type ,(cl :x))
              (ref.b b :type ,(cl :x))
              (ref.r ret :type ,(tensor 'boolean)))
             (setf ref.r (t.f= ,(field-type (cl :x)) ref.a ref.b)))
     ret))

(define-tensor-method ga= (a (b dense-tensor :x))
  `(let ((a (t.coerce ,(field-type (cl :x)) a))
         (ret t))
     (with-memoization ()
       (loop for idx being the idx from 0 below (dimensions b) 
             with-iterator (:stride ((of-b (strides b) (head b))))
             do (unless (t.f= ,(field-type (cl :x)) a (t.store-ref ,(cl :x) (memoizing (store b) :type ,(store-type (cl :x))) of-b))
                  (setf ret nil)
                  (return))))
     ret))

(define-tensor-method ga= ((a dense-tensor :x) (b dense-tensor :x))
  `(let ((ret t))
     (with-memoization ()
       (loop for idx being the idx from 0 below (dimensions b) with-iterator (:stride ((of-a (strides a) (head a))
                                                                               (of-b (strides b) (head b))))
             do (unless (t.f= ,(field-type (cl :x))
                              (t.store-ref ,(cl :x) (memoizing (store a) :type ,(store-type (cl :x))) of-a)
                              (t.store-ref ,(cl :x) (memoizing (store b) :type ,(store-type (cl :x))) of-b))
                  (setf ret nil)
                  (return))))
     ret))

(defmethod ga= ((a null) (b #.(tensor 'boolean)))
  (declare (ignore a))
  (let ((ret t))
    (with-memoization ()
      (loop for idx being the idx from 0 below (dimensions b) with-iterator (:stride ((of-b (strides b) (head b))))
            do (unless (t.store-ref #.(tensor 'boolean) (memoizing (store b) :type #.(store-type (tensor 'boolean))) of-b)
                 (setf ret nil)
                 (return))))
    ret))

(define-tensor-method go= (a (b dense-tensor :x))
  `(let ((a (t.coerce ,(field-type (cl :x)) a))
         (ret nil))
     (with-memoization ()
       (loop for idx being the idx from 0 below (dimensions b) with-iterator (:stride ((of-b (strides b) (head b))))
             do (when (t.f= ,(field-type (cl :x)) a (t.store-ref ,(cl :x) (memoizing (store b) :type ,(store-type (cl :x))) of-b))
                  (setf ret t)
                  (return))))
     ret))

(define-tensor-method go= ((a dense-tensor :x) (b dense-tensor :x))
  `(let ((ret nil))
     (with-memoization ()
       (loop for idx being the idx from 0 below (dimensions b) with-iterator (:stride ((of-a (strides a) (head a))
                                                                                       (of-b (strides b) (head b))))
             do (when (t.f= ,(field-type (cl :x))
                            (t.store-ref ,(cl :x) (memoizing (store a) :type ,(store-type (cl :x))) of-a)
                            (t.store-ref ,(cl :x) (memoizing (store b) :type ,(store-type (cl :x))) of-b))
                  (setf ret t)
                  (return))))
     ret))

(defmethod go= ((a null) (b #.(tensor 'boolean)))
  (declare (ignore a))
  (let ((ret nil))
    (with-memoization ()
      (loop for idx being the idx from 0 below (dimensions b) 
            with-iterator (stride ((of-b (strides b) (head b))))
            do (when (t.store-ref #.(tensor 'boolean) 
                                  (memoizing (store b) 
                                             :type #.(store-type (tensor 'boolean)))
                                  of-b)
                 (setf ret t)
                 (return))))
      ret))
