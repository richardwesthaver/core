...
(defun print-tensor (tensor stream)
  (let ((rank (rank tensor))
    ...
    (labels ((two-print (tensor subs)
               (let ((strs) (cprints)
                     (maxw (make-array (if (eq *print-tensor-max-len* t) (aref dims 1) *print-tensor-max-len*) :initial-element 0)))
                 (setq strs
                 ...
                 (loop for row in strs
                       ...
                       do (loop for cref in row
                                for j from 0 below (length maxw)
                                do (format stream (replace (make-string (+ (aref maxw j) *print-tensor-indent*) :initial-element #\Space) cref :start1 (if (char= (aref cref 0) #\-) 1 0))))
                       do (format stream "~%"))
