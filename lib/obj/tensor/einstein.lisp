...
(defun parse-loopx (type place clause)
  (let* ((refs (let ((tmp (getcons (list place clause) 'ref))
         ...
         (tlist (mapcar #'(lambda (sym)
                            (let* ((gsym sym)
                                   (hsym (gensym (concatenate 'string "head-" (symbol-name sym)))))
                              `(:tensor (,gsym ,sym :type ,type)
                                :head (,hsym (head ,gsym) :type index-type)
                                :store (,(gensym (concatenate 'string "store-" (symbol-name sym))) (t/store ,type ,gsym) :type ,(store-type type))
                                :strides (,(gensym (concatenate 'string "strides-" (symbol-name sym))) (strides ,gsym) :type index-store-vector)
                                :dimensions (,(gensym (concatenate 'string "dimensions-" (symbol-name sym))) (dimensions ,gsym) :type index-store-vector))))
                        tens))
         (indices (let ((tmp nil)
      ...
      (labels ((get-prop (x &optional prop)
        ...
        (loop :for ref :in refs
           :do (let* ((plist (get-prop (second ref)))
                      (ofsym (gensym (concatenate 'string "offset-" (symbol-name (second (getf plist :tensor))))))
                      (ret `((,ofsym ,(car (getf plist :head)) :type index-type)
                             (,(gensym (concatenate 'string "ref-" (symbol-name (second (getf plist :tensor))))) (t/store-ref ,type ,(car (getf plist :store)) ,ofsym) :type ,(field-type type)))))
                           ...
                           (cond
                             ...
                             (t
                              (let* ((plst (get-prop ten))
                                     (dsym (gensym (concatenate 'string "d-stp-" (symbol-name cidx) "-" (symbol-name ten))))
