...
(defmethod load-config ((fmt (eql :box)) from &key)
  (with-safe-io-syntax (:box)
    (let* ((ast (read-lisp-file from))
      ...
      (cond 
        ...
        (class (make-config :box :class class :ast ast :from %from))
        (type (make-config :box :class (find-symbol (concatenate 'string (string-upcase type) "-CONFIG") :box) :ast ast :from %from))
        (t (make-config :box :ast ast :from %from))))))
