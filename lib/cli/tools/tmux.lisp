...
(defmethod make-config ((obj (eql :tmux)) &key commands server session window keys)
  (let ((config (make-instance 'tmux-config)))
    ...
    (when server (setf (slot-value config 'server-options) (plist-string-hash-table server)))
    (when session (setf (slot-value config 'session-options) (plist-string-hash-table session)))
    (when window (setf (slot-value config 'window-options) (plist-string-hash-table window)))
    (when keys (setf (slot-value config 'keys) (plist-string-hash-table keys)))
    config))
