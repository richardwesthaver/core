...
;; For example see the DAT/X509 package.
...
(defun asn1-iastring-char-p (char)
  (< (char-code char) #x80))
...
(defmethod decode-asn1-string ((self string) (type (eql +v-asn1-iastring+)))
  (if (asn1-iastring-p self)
      (sb-ext:string-to-octets self :external-format :ascii)
      (error 'invalid-asn1-string)))
...
(defmethod decode-asn1-string (self (type (eql +v-asn1-utf8string+)))
  (sb-ext:string-to-octets self :external-format :utf8))

(defmethod decode-asn1-string (self (type (eql +v-asn1-universalstring+)))
  (if (= 0 (mod (length self) 4))
      (sb-ext:string-to-octets self :external-format :utf32)
...
(defmethod decode-asn1-string (self (type (eql +v-asn1-bmpstring+)))
  (if (= 0 (mod (length self) 2))
      (sb-ext:string-to-octets self :external-format :utf-16/be)
...
(defun decode-asn1-time (asn1-time)
  ...
  (let ((time-string (sb-ext:octets-to-string (sb-ext:string-to-octets asn1-time)
                                              :external-format :ascii)))
    (let* ((utctime-p #+nil (= 1 (asn1-utctime-check asn1-time)))
