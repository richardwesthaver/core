...
(define-condition multipart-parsing-error (http-error) ())
...
(definline byte-to-ascii-lower (x)
...
(definline ascii-octets-to-string (octets &key (start 0) (end (length octets)))
...
(definline ascii-octets-to-lower-string (octets &key (start 0) (end (length octets)))
  (declare (type octet-vector octets)
           (type (unsigned-byte 64) start end)
           (optimize (speed 3) (safety 0)))
  (let* ((len (the (unsigned-byte 64) (- end start)))
        ...
        (setq callbacks
              (make-callbacks
               ...
               :body (lambda (parser data start end)
                       (declare (ignore parser))
                       (write-sequence data body-buffer start end)))))
      (lambda (data)
