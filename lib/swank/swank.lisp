  ...
  (let (connection)
    (unwind-protect
      ...
      (unless dont-close
        ...
        (when (and connection (eq style :spawn))
...
(defvar *swank-wire-protocol-version* "1.0.0"
...
;; TODO 2026-05-01: 
(defslimefun load-script-file (filename)
  (with-buffer-syntax ()
    (to-string (load (filename-to-pathname filename)))))
...
(defvar *find-module* 'find-swank-module
...
(defun module-candidates (name dir)
  (list (std::fasl-cache-file (make-pathname :name name :type "lisp" :defaults dir))
        (make-pathname :name name :type "lisp" :defaults dir)))

(defun find-swank-module (module)
