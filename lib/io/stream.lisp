...
(with-memoization ()
  (memoizing
   (defun buffer-stream (length)
     (or (std/macs:if-let ((class (find length (std/meta:class-direct-subclasses (find-class 'buffer-stream)) :key (lambda (x) (slot-value x 'length)))))
           (class-name class)
...
(defmethod free ((self buffer-stream))
  (unless (or (not (buffer self)) (null-alien (buffer self)))
    (setf (buffer self) (foreign-free (buffer self)))))
...
(defun bsref (x i)
  ...
  (let ((n (buffer-stream-length x)))
    ...
    (sap-svref (slot-value x 'buffer) 'unsigned-char i)))
        ...
        ((the fv obj)
         (with-gensyms (obj-v i-v n-v)
             `(lety ((,obj-v ,obj :type ,fv)
                ...
                (sap-svref (slot-value (the ,fv ,obj-v) 'buffer) 'unsigned-char (the fixnum (* (the fixnum ,i-v) (the fixnum 1)))))))
        ((t) form))
      form))
...
(defun (setf bsref) (value x i)  
  ...
  (let ((n (buffer-stream-length x)))
    (assert (< -1 i n) nil 'out-of-bounds-error :requested i :bound n)
    (setf (sap-svref (slot-value x 'buffer) 'unsigned-char i) value)))
            ...
            `(lety ((,obj-v ,obj :type ,fv)
               ...
               ,@(if *bsref-range-check*
                     `((let ((,n-v (slot-value (class-of ,obj-v) 'length)))
                         (assert (< -1 ,i-v ,n-v) nil 'out-of-bounds-error :requested ,i-v :bound ,n-v))))
               (setf (sap-svref (slot-value (the ,fv ,obj-v) 'buffer) 'unsigned-char (the fixnum ,i-v))
                     (the ,lt ,val)))))
      form))
...
(define-io :buffer
   ...
   (:write (b bs)
           ...
           (with-slots (size) bs
             ...
             (let ((needed (the fixnum (+ size 1))))
               ...
               (when (> needed (the fixnum (buffer-stream-length bs))) 
                 (setf bs (resize-buffer-stream bs needed)))
  ...
  (((signed-byte 32) :alias int32)
   ...
   (:read (bs)
          ...
          (let ((position (the fixnum (offset bs))))
            ...
            (the (signed-byte 32) (sap-svref (buffer bs) 'int position)))))
  (fixnum32
   (:read (bs)
          ...
          (let ((position (the fixnum (offset bs))))
            (declare (fixnum position))
            (setf (offset bs) (the fixnum (+ position 4)))
            (the fixnum (sap-svref (buffer bs) 'int position))))
   (:write (i bs)
           ...
           (let ((buf (buffer bs))
             ...
             (let ((needed (the fixnum (+ size 4))))
               (declare (fixnum needed))
               (when (> needed len) (setf bs (resize-buffer-stream bs needed)))
               (setf (sap-svref buf 'int size) i
                     (size bs) needed)
               nil))))
  (((unsigned-byte 32) :alias uint32)
   (:read (bs)
          ...
          (let ((position (the fixnum (offset bs))))
            (declare (fixnum position))
            (setf (offset bs) (the fixnum (+ position 4)))
            (sap-svref (buffer bs) 'unsigned-int position)))
   (:write (u bs)
           ...
           (let ((buf (buffer bs))
             ...
             (let ((needed (the fixnum (+ size 4))))
               ...
               (when (> needed len)
                 (setf bs (resize-buffer-stream bs needed)))
               (setf (sap-svref buf 'unsigned-int size) u
                     (size bs) needed)
               nil))))
  (((signed-byte 64) :alias int64)
   (:read (bs)
          ...
          (let ((position (offset bs)))
            ...
            (the (signed-byte 64) (sap-svref (buffer bs) 'long position))))
   (:write (i bs)
           ...
           (let ((buf (buffer bs))
             ...
             (let ((needed (the fixnum (+ size 8))))
               ...
               (when (> needed len)
                 (setf bs (resize-buffer-stream bs needed)))
               (setf (sap-svref buf 'long size) i
                     (size bs) needed)
               nil))))
  (fixnum64
   (:read (bs)
          ...
          (let ((position (offset bs)))
            ...
            (sap-svref (buffer bs) 'long position)))
   (:write (i bs)
           "Write a 64-bit signed integer."
           (declare (buffer-stream bs)
                    (fixnum i))
  ...
  (((unsigned-byte 64) :alias uint64)
   (:read (bs)
          ...
          (let ((position (offset bs)))
            (declare (fixnum position))
            (setf (offset bs) (the fixnum (+ position 8)))
            (the (unsigned-byte 64) (sap-svref (buffer bs) 'unsigned-long position))))
   (:write (u bs)
           ...
           (let ((buf (buffer bs))
             ...
             (let ((needed (the fixnum (+ size 8))))
               ...
               (when (> needed len)
                 (setf bs (resize-buffer-stream bs needed)))
               (setf (sap-svref buf 'unsigned-long size) u
                     (size bs) needed)
               nil))))
  ((single-float :alias float)
   (:read (bs)
    ...
    (declare (fixnum position))
    (setf (offset bs) (the fixnum (+ position 4)))
            (the single-float (sap-svref (buffer bs) 'single-float position))))
   (:write (d bs)
           ...
           (let ((buf (buffer bs))
             ...
             (let ((needed (the fixnum (+ size 4))))
               ...
               (when (> needed len)
                 (setf bs (resize-buffer-stream bs needed)))
               (setf (sap-svref buf 'single-float size) d
                     (size bs) needed)
               nil))))
  ((double-float :alias double)
   (:read (bs)
          ...
          (let ((position (offset bs)))
            (declare (fixnum position))
            (setf (offset bs) (the fixnum (+ position 8)))
            (the double-float (sap-svref (buffer bs) 'double-float position))))
   (:write (d bs)
           ...
           (let ((buf (buffer bs))
             ...
             (let ((needed (the fixnum (+ size 8))))
               ...
               (when (> needed len)
                 (setf bs (resize-buffer-stream bs needed)))
               (setf (sap-svref buf 'double-float size) d
                     (size bs) needed)
               nil))))
  (octet-vector
   (:read (bs)
