...
(defmethod alloc ((self buffer-stream))
  (setf (buffer self) 
        #+nil (foreign-alloc `(array unsigned-char ,(buffer-stream-length self)))
        (make-alien unsigned-char (buffer-stream-length self))))

(defmethod free ((self buffer-stream))
  (unless (or (not (buffer self)) (null-alien (buffer self)))
    (setf (buffer self) (free-alien (buffer self)))))
...
(defun bsref (x i)
  ...
  (let ((n (slot-value (class-of (the buffer-stream x)) 'length)))
    ...
    ;; using SAP here, but alien-value in IO..
    (sap-svref (alien-sap (slot-value x 'buffer)) 'sb-alien:unsigned-char i)))
          ...
          (with-gensyms (obj-v i-v n-v)
            `(lety ((,obj-v ,obj :type ,fv)
               ...
               ,@(if *bsref-range-check*
                     `((let ((,n-v (slot-value (class-of ,obj-v) 'length)))
                         (assert (< -1 ,i-v ,n-v) nil 'out-of-bounds-error :requested ,i-v :bound ,n-v))))
               (setf (sap-ref (alien-sap (slot-value (the ,fv ,obj-v) 'buffer))
            ...
            (when (>= vlen 0)
              (let ((v (make-array vlen :element-type '(unsigned-byte 8))))
                  (with-vector-sap (dst-ptr v)
                    (memcpy dst-ptr (sb-sys:sap+ (alien-sap (buffer bs)) position) vlen)
                    v)))))
   (:write (bv bs)
