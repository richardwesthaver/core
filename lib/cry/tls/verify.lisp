;;; verify.lisp --- Hostname Verification
  ...
  (if single-char-wildcard
      (let ((pattern-except-left-most-label
             (if-let ((first-hostname-dot-post (position #\. pattern)))
               ...
               pattern)))
        (string-equal after-wildcard pattern-except-left-most-label))
      (when (wildcard-not-in-a-label before-wildcard after-wildcard)
        ;; baz*.example.net and *baz.example.net and b*z.example.net would
        ;; be taken to match baz1.example.net and foobaz.example.net and
        ;; buzz.example.net, respectively
        (and
         (string-equal before-wildcard (subseq pattern 0 (length before-wildcard)))
         (string-equal after-wildcard (subseq pattern
                                                        (- (length pattern)
